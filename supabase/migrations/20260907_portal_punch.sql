-- portal_punch: the employee mobile app's clock-in/out RPC.
--
-- SECURITY DEFINER, mirrors the shape of portal_get_shifts. The only identity this function
-- ever trusts is whatever _resolve_portal_session(p_session) resolves to -- there is no
-- p_employee_id parameter, and there never should be one. A phone logged in as Employee A has
-- no code path that can punch as Employee B; the session is the only source of "who is this".
--
-- p_punch_date/p_punch_time are supplied by the client, matching the kiosk's existing convention
-- (index.html's executePunch computes these from the device's own local time, not server-derived
-- UTC) -- there is no per-site timezone column to convert a UTC instant back to a correct local
-- calendar date server-side, so consistency with the existing convention is the only sound choice.
-- p_client_time is a separate, true instant (with timezone) used only to timestamp the row
-- (punched_at) and to detect an offline-queued sync -- it is not used to derive the date/time
-- strings above.
